...
export const AdminNewThemeGroup = () => {
    ...
    const { grupos, createNuevoGrupo, cargando } = useThemeGroupsStore();

    const [nombreGrupo, setNombreGrupo] = useState('');
    const [colores, setColores] = useState<Theme[]>([]);
    const [error, setError] = useState<string | null>(null);
    const [success, setSuccess] = useState<boolean>(false);

    // 🟣 Cargar temas predeterminados desde BD
    useEffect(() => {
        const predeterminado = grupos.find((g) => g.nombre === 'Predeterminado');
        if (predeterminado) {
            // clonar para edición sin mutar original
            setColores(predeterminado.colores.map(c => ({ ...c })));
        } else {
            setError('No se encontró el tema predeterminado en la base de datos.');
        }
    }, [grupos]);
